...
export const DisplayNewReview = ({ review, setNewReview }) => {
 ...
 const navigate = useNavigate()
  ...
  const addDefaultSrc = e => {
    ...
    setImageMessage('Either you did not provide an image or the URL did not work, so a default image has been set.')
}
  ...
  return (
    ...
    <section className='singleReview newReview'>
      ...
      <section>
      {message==='Your review was successfully added!' ? <button onClick={()=>navigate('/')} >Back to home</button> :<button className='newReviewSubmit' onClick={submitForm}>Confirm</button>}
